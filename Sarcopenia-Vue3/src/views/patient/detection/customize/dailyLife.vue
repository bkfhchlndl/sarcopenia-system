    ...
    <header class="top-bar">
      <div class="top-left">
        ...
        <div class="title-block">
          <h1>日常生活活动能力评估</h1>
          <p>Barthel ADL 评定量表 · 共 {{ scaleData.questionList?.length || 0 }} 项</p>
        </div>
      </div>
      <div class="top-right">
      ...
      <div class="result-card standard-card">
        ...
        <div class="standard-list">
          <div
              ...
              :key="index"
              class="stan-row"
              :class="{ 'is-current': currentLevelIndex === index }"
          >
            <span class="stan-dot" :style="{ background: level.color }"></span>
            <span class="stan-range">
              {{ index === 0 ? '≤ 40 分' : index === 3 ? '100 分' : `${40 * index + 1} ~ ${level.maxScore} 分` }}
            </span>
...
<script setup name="DailyLifeAssessment">
...
const currentLevelIndex = computed(() => {
  ...
  for (let i = 0; i < DEPENDENCY_LEVELS.length; i++) {
    if (score <= DEPENDENCY_LEVELS[i].maxScore) {
      return i
    }
  }
...
const dependencyLevel = computed(() => {
  ...
  return idx >= 0 ? DEPENDENCY_LEVELS[idx] : DEPENDENCY_LEVELS[0]
})

/** 当前等级对应评估建议 */
const dependencySuggest = computed(() => dependencyLevel.value.suggest)

// ==================== 工具函数 ====================
/**
 * 从多个候选值中提取有效数字ID
 * @param  {...any} values 待判断的候选值
 * @returns {number|null} 有效数字ID或null
 */
...
function backToComprehensive() {
  // 写入完成标记，自定义列表页将据此显示"已完成"状态
  try {
    if (patientId.value) {
      localStorage.setItem(`${STORAGE_PREFIX}${patientId.value}:basic_adl`, '1')
    }
  } catch (e) {}
...
async function submitAssessment() {
  ...
  if (answeredCount.value < total) {
    ElMessage.warning(`还有 ${total - answeredCount.value} 道题未作答`)
    return
  }

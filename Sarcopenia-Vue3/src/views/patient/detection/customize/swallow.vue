    ...
    <section class="result-area">
      ...
      <div class="result-card findings-card">
        <div class="rc-head">
          ...
          <span>评估结果 · {{ swallowLevel.text }}</span>
        </div>
...
import { selectSwallowScale, insertCgaRecord } from '@/api/cga.js'
...
const targetScaleId = toNumberId(route.query.targetScaleId, route.query.projectId, route.query.scaleId) || 35
...
const loadScaleData = async () => {
  try {
    const res = await selectSwallowScale()
...
function isOptionNormal(option) {
  ...
  const normalKeywords = ['正常', '无', '完全', '好', '没有', '良好', '顺利', '自如', '不困难', '可以', '能够']
...
const swallowLevel = computed(() => {
  ...
  if (abnormal === 0) return { text: '吞咽功能正常', color: '#22c55e' }
  if (abnormal <= Math.ceil(total * 0.3)) return { text: '轻度吞咽困难', color: '#eab308' }
  if (abnormal <= Math.ceil(total * 0.6)) return { text: '中度吞咽困难', color: '#f97316' }
  return { text: '重度吞咽困难', color: '#ef4444' }
})
...
const combinedSuggest = computed(() => {
  ...
  const level = swallowLevel.value.text
  if (level === '吞咽功能正常') {
    return '目前吞咽功能评估未见明显异常。建议保持良好的进食习惯，进食时集中注意力，细嚼慢咽，避免边吃边说话，保持口腔卫生。'
  }
  if (level === '轻度吞咽困难') {
    return '存在轻度吞咽困难。建议调整饮食质地（软食、糊状食物），进食时采取坐位或半卧位，小口慢咽，每次吞咽后清嗓确认，必要时进行吞咽功能训练。'
  }
  if (level === '中度吞咽困难') {
    return '存在中度吞咽困难。建议立即进行专业吞咽评估（如洼田饮水试验、视频荧光造影检查），调整饮食为糊状或稠厚食物，必要时进行吞咽康复训练，警惕误吸和吸入性肺炎风险。'
  }
  // 重度
  return '存在重度吞咽功能障碍，强烈建议立即就诊进行专业吞咽评估（VFSS/FEES），可能需要鼻胃管喂养或经皮胃造瘘，配合专业吞咽康复治疗，严格预防误吸和吸入性肺炎。'
})

// 提交到数据库的 result 文本（精简版）
const resultText = computed(() => {
  if (answeredCount.value < totalQuestions.value) return ''
  return `${swallowLevel.value.text}。${combinedSuggest.value.split('。')[0]}。`
})
...
const backToComprehensive = () => {
  try {
    if (patientId) {
      localStorage.setItem(`customize_done:${patientId}:swallow`, '1')
    }
  } catch (e) {}
  router.push({

...
async function submitBasicInfo() {
  ...
  try {
    ...
    // 标记本地完成状态
    try {
      localStorage.setItem(`${STORAGE_DONE_PREFIX}${patientId.value}:disease_history`, '1')
      localStorage.setItem(`${STORAGE_DONE_PREFIX}${patientId.value}:basic_information`, '1')
    } catch (e) {
    }

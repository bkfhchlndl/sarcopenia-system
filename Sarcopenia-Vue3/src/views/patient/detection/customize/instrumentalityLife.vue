    ...
    <header class="top-bar">
      <div class="top-left">
        ...
        <div class="title-block">
          <h1>工具性日常生活活动能力评估</h1>
          <p>Lawton IADL 量表 · 共 {{ scaleData.questionList?.length || 0 }} 项</p>
        </div>
      </div>
      <div class="top-right">
        ...
        <div class="standard-list">
          ...
          >
            ...
            <span class="stan-range">
              {{ index === 0 ? '≤ 2 分' : index === 3 ? '8 分' : `${2 * index + 1} ~ ${level.maxScore} 分` }}
            </span>
...
<script setup name="InstrumentalityLifeAssessment">
import { ref, computed, onMounted } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { ArrowLeft, Check, CircleCheck } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import { selectInstrumentalityLifeScale, insertCgaRecord } from '@/api/cga.js'
...
async function loadScaleData() {
  try {
    const res = await selectInstrumentalityLifeScale()
...
function backToComprehensive() {
  try {
    if (patientId.value) {
      localStorage.setItem(`${STORAGE_PREFIX}${patientId.value}:instrumental_adl`, '1')
    }
  } catch (e) {}
  router.push({

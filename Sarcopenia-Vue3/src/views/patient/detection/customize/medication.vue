    ...
    <section class="result-area" v-if="answeredCount === totalQuestions">
      ...
      <div class="result-card note-card" style="grid-column: 1 / -1;">
        ...
        <div class="note-block" v-if="textItemsText">
          <div class="note-title"><span class="dot-normal" style="color:#14b8a6;">●</span> 附加描述</div>
...
function isTextQuestion(question) {
  // type === 3 表示文本题（与其他量表组件一致），
  // 或 optionList 为空，或所有选项内容都为空
  if (question.type === 3) return true
...
// 文本题为选填，不计入"已完成"的必填统计
function isQuestionAnswered(question) {
  if (isTextQuestion(question)) {
    const v = answers.value[question.questionId]
    return v !== undefined && v !== null && String(v).trim().length > 0
  }
  return answers.value[question.questionId] !== undefined && answers.value[question.questionId] !== null
}

// ============ 进度 ============
const totalQuestions = computed(() => {
  // 仅统计"选择题"数量（文本题为选填，不计入总数）
...
const answeredCount = computed(() => {
  ...
  for (const q of (scaleData.value.questionList || [])) {
    ...
    if (answers.value[q.questionId] !== undefined && answers.value[q.questionId] !== null) count++
  }
  return count
})
...
function getSelectedContent(qIndex) {
  ...
  if (!q) return ''
  if (isTextQuestion(q)) {
    const text = answers.value[q.questionId]
...
const textItemsText = computed(() => {
  ...
  for (let i = 0; i < (scaleData.value.questionList || []).length; i++) {
    ...
    if (!isTextQuestion(q)) continue
    const content = getSelectedContent(i)
    if (content) list.push(`${i + 1}. ${q.title}：${content}`)
  }
  return list.join('；')
})
...
const conclusionText = computed(() => {
  ...
  if (risk.length <= 2) {
    return `本次多重用药评估共发现 ${risk.length} 项用药相关问题，存在轻度多重用药风险。`
  }
  if (risk.length <= 4) {
    return `本次多重用药评估共发现 ${risk.length} 项用药相关问题，存在中度多重用药风险。`
  }
  return `本次多重用药评估共发现 ${risk.length} 项用药相关问题，存在高度多重用药风险，建议药物重整。`
})
...
const submitAssessment = async () => {
  ...
  // 注意：answers 只能存选项ID数字（后端是 Map<Long, List<Long>>），文本题跳过
  const formattedAnswers = {}
  for (const q of (scaleData.value.questionList || [])) {
    const qid = String(q.questionId)
    const raw = answers.value[q.questionId]
    // 文本题不进 answers，否则后端会按 Long 解析失败
    if (isTextQuestion(q)) continue
    // 选择题 —→ 存选项ID
...
.back-btn {
  ...
  padding: 6px 12px;
  border-radius: 8px;
  &:hover { color: #14b8a6; background: #ccfbf1; }
}
...
.question-card {
  ...
  &.is-answered { border-color: #5eead4; }
}
...
.question-card.is-answered .q-index { background: #14b8a6; color: #fff; }
...
.q-textarea-wrap .el-textarea__inner::placeholder { color: #94a3b8; }
...
.q-textarea-tip {
  ...
  .tip-ok { color: #0f766e; font-weight: 600; }
  .tip-empty { color: #94a3b8; }
}
...
.q-option {
  ...
  &.is-selected { background: #ccfbf1; border-color: #14b8a6; box-shadow: 0 2px 8px rgba(20,184,166,0.14); }
}
...
.opt-radio {
  ...
  &.is-checked { color: #14b8a6; }
}

.opt-label { font-size: 14px; color: #374151; line-height: 1.5; }
.is-selected .opt-label { color: #115e59; font-weight: 600; }
...
.submit-area .el-button--primary {
  ...
  &:hover { box-shadow: 0 6px 20px rgba(13,148,136,0.4); transform: translateY(-1px); }

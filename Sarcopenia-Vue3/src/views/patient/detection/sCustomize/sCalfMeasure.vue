<template>
  <div class="calf-measure-container">
    <!-- 顶部导航栏 -->
    <header class="top-bar">
      <div class="top-left">
        ...
        <div class="title-block">
          <h1>小腿围测量</h1>
          <p>Calf Circumference Measurement · 肌少症评估</p>
        </div>
      </div>
      <div class="top-right">
    ...
    <div class="main-content">
      ...
      <div class="left-panel">
        ...
        <!-- 平均值 + 风险等级 -->
        <div class="result-card" v-if="averageValue !== null">
          <div class="result-left">
            <div class="result-label">平均值</div>
            <div class="result-value">
              <span class="rv-num">{{ averageValue }}</span>
              <span class="rv-unit">cm</span>
            </div>
          </div>
          ...
          <div v-else class="history-list">
            ...
            >
              ...
              <div class="hi-values">
                ...
                <div class="hv-item avg">
                  <span class="hv-label">平均</span>
                  <span class="hv-num">{{ calcAverage(record) }}</span>
                  <span class="hv-unit">cm</span>
                </div>
              </div>
        ...
        <div class="reference-card">
          ...
          <div class="ref-list">
            <div class="ref-item">
              ...
              <span class="ref-label">正常范围</span>
              <span class="ref-value">≥ 34 cm</span>
            </div>
            <div class="ref-item">
              <span class="ref-dot warning"></span>
              <span class="ref-label">临界范围</span>
              <span class="ref-value">33 ~ 34 cm</span>
            </div>
            <div class="ref-item">
              <span class="ref-dot danger"></span>
              <span class="ref-label">肌少症风险</span>
              <span class="ref-value">＜ 33 cm</span>
            </div>
          </div>
          <div class="ref-tip">
            小腿围是肌少症筛查的重要体格指标，男性 &lt; 34cm、女性 &lt; 33cm 提示肌少症可能。
          </div>
        </div>
      </div>
    </div>
...
<script setup name="SCalfMeasure">
...
import { selectCalfMeasureList, insertCalfMeasure } from '@/api/s.js'
...
/**
 * 肌少症风险等级（基于小腿围）
 * 正常 ≥34cm，临界 33-34cm，风险 <33cm
 */
const riskLevel = computed(() => {
  if (averageValue.value == null) {
    return { text: '待评估', type: 'info', desc: '请完成测量后查看风险评估' }
  }
  const val = Number(averageValue.value)
  if (val < 33) {
    return {
      text: '肌少症高风险',
      type: 'danger',
      desc: '小腿围低于正常范围，建议进一步进行肌力、肌量和躯体功能评估'
    }
  }
  if (val < 34) {
    return {
      text: '临界范围',
      type: 'warning',
      desc: '小腿围处于临界值，建议加强运动和营养摄入，定期监测'
    }
  }
  return {
    text: '正常范围',
    type: 'success',
    desc: '小腿围在正常范围内，继续保持健康生活方式'
  }
})
...
async function handleSubmit() {
  ...
  try {
    ...
    const res = await insertCalfMeasure(submitData)
    if (res && res.code === 200) {
...
function backToCustomize() {
  try {
    if (patientId.value) {
      localStorage.setItem(`sarcopenia_customize_done:${patientId.value}:calf_measure`, '1')
    }
  } catch (e) {
